...
public class MultiBufferProductPlan implements Plan {
	...
	/**
	 * A scan for this query is created and returned, as follows. First, the method
	 * materializes its RHS query. It then determines the optimal chunk size, based
	 * on the size of the materialized file and the number of available buffers. It
	 * creates a chunk plan for each chunk, saving them in a list. Finally, it
	 * creates a multiscan for this list of plans, and returns that scan.
	 * 
	...
	/**
	 * Returns an estimate of the number of block accesses required to execute the
	 * query. The formula is:
	 * 
	 * <pre>
	 * B(product(p1, p2)) = B(p2) + B(p1) * C(p2)
	 * </pre>
	 * 
	 * where C(p2) is the number of chunks of p2. The method uses the current number
	 * of available buffers to calculate C(p2), and so this value may differ when
	 * the query scan is opened.
	 * 
	 ...
	 * product does not increase or decrease field values, the estimate is the same
	 * as in the appropriate underlying query.
	 * 
	...
	/**
	 * Returns the schema of the product, which is the union of the schemas of the
	 * underlying queries.
	 * 
