...
public class Page {
	/**
	 * The number of bytes in a block. This value is set unreasonably low, so that
	 * it is easier to create and test databases having a lot of blocks. A more
	 * realistic value would be 4K.
	 */
	public static final int BLOCK_SIZE = 400;

	/**
	 * The size of an integer in bytes. This value is almost certainly 4, but it is
	 * a good idea to encode this value as a constant.
	 */
	public static final int INT_SIZE = Integer.SIZE / Byte.SIZE;

	/**
	 * The maximum size, in bytes, of a string of length n. A string is represented
	 * as the encoding of its characters, preceded by an integer denoting the number
	 * of bytes in this encoding. If the JVM uses the US-ASCII encoding, then each
	 * char is stored in one byte, so a string of n characters has a size of 4+n
	 * bytes.
	 * 
	...
	/**
	 * Creates a new page. Although the constructor takes no arguments, it depends
	 * on a {@link FileMgr} object that it gets from the method
	 * {@link simpledb.server.SimpleDB#fileMgr()}. That object is created during
	 * system initialization. Thus this constructor cannot be called until either
	 * {@link simpledb.server.SimpleDB#init(String)} or
	 * {@link simpledb.server.SimpleDB#initFileMgr(String)} or
	 * {@link simpledb.server.SimpleDB#initFileAndLogMgr(String)} or
	 * {@link simpledb.server.SimpleDB#initFileLogAndBufferMgr(String)} is called
	 * first.
	 */
	public Page() {
	}
	...
	/**
	 * Returns the integer value at a specified offset of the page. If an integer
	 * was not stored at that location, the behavior of the method is unpredictable.
	 * 
	...
	/**
	 * Returns the string value at the specified offset of the page. If a string was
	 * not stored at that location, the behavior of the method is unpredictable.
	 * 
	 * @param offset the byte offset within the page
