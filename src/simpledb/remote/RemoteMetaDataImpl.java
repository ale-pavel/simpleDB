...
public class RemoteMetaDataImpl extends UnicastRemoteObject implements RemoteMetaData {
	 ...
	 * numbers start with 1, so the field is taken from position (column-1) in the
	 * list.
	 * 
	...
	/**
	 * Returns the type of the specified column. The method first finds the name of
	 * the field in that column, and then looks up its type in the schema.
	 * 
	 ...
	 * For a string-type field, the method simply looks up the field's length in the
	 * schema and returns that. For an int-type field, the method needs to decide
	 * how large integers can be. Here, the method arbitrarily chooses 6 characters,
	 * which means that integers over 999,999 will probably get displayed
	 * improperly.
	 * 
