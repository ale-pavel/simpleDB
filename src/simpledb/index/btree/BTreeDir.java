...
public class BTreeDir {
	...
	/**
	 * Returns the block number of the B-tree leaf block that contains the specified
	 * search key.
	 * 
	...
	/**
	 * Creates a new root block for the B-tree. The new root will have two children:
	 * the old root, and the specified block. Since the root must always be in block
	 * 0 of the file, the contents of the old root will get transferred to a new
	 * block.
	 * 
	...
	/**
	 * Inserts a new directory entry into the B-tree block. If the block is at level
	 * 0, then the entry is inserted there. Otherwise, the entry is inserted into
	 * the appropriate child node, and the return value is examined. A non-null
	 * return value indicates that the child node split, and so the returned entry
	 * is inserted into this block. If this block splits, then the method similarly
	 * returns the entry information of the new block to its caller; otherwise, the
	 * method returns null.
	 * 
