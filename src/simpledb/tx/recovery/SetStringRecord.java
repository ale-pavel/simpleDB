...
class SetStringRecord implements LogRecord {
	...
	/**
	 * Writes a setString record to the log. This log record contains the SETSTRING
	 * operator, followed by the transaction id, the filename, number, and offset of
	 * the modified block, and the previous string value at that offset.
	 * 
	...
	/**
	 * Replaces the specified data value with the value saved in the log record. The
	 * method pins a buffer to the specified block, calls setString to restore the
	 * saved value (using a dummy LSN), and unpins the buffer.
	 * 
