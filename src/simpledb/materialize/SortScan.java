...
public class SortScan implements Scan {
	...
	/**
	 * Positions the scan before the first record in sorted order. Internally, it
	 * moves to the first record of each underlying scan. The variable currentscan
	 * is set to null, indicating that there is no current scan.
	 * 
	...
	/**
	 * Moves to the next record in sorted order. First, the current scan is moved to
	 * the next record. Then the lowest record of the two scans is found, and that
